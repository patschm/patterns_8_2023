...
{
    static ConsoleDevice device = new ConsoleDevice();
    ...
    static Triangle CreateTriangle(int basis = 200, int hoogte=100, int hoek=30)
    {
        ...
        return new Triangle { Location = pos, Base=basis, Angle = hoek, Heigth = hoogte};
    }
    static Circle CreateCircle(int straal = 100)
    {
        Position pos = RandomPosition();
        return new Circle { Location = pos, Radius = straal };
    }
    static Rectangle CreateRectangle(int hoogte = 100, int breedte = 200)
    {
        Position pos = RandomPosition();
        return new Rectangle { Location = pos, Height = hoogte, Width = breedte };
    }
